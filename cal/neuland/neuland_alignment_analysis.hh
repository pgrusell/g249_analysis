...
public:
    neulandAlignmentAnalysis(TString resultsFile = "")
    {
        ...
        hToFFull = static_cast<TH2D *>(histDataFile->Get("hCorrected_fullToF"));
        ...
        // Full ToF
        printTofFull(resultsPath);

        // Compare Full
        compareTofFull(resultsPath);
    }
    ...
    static void printCanvas(TH2D *&h, TString title, TString xTit, TString yTit, TString path, double ymin = 0, double ymax = 0, bool logy = false)
    {
    ...
    void printTofFull(TString path)
    {
        auto *pr = hToFFull->ProjectionY("pr");
        setHistogramStyle(pr, "ToF [ns]", "# counts", kCyan - 6, true);
        auto *c = new TCanvas("c", "hTofFull", 800, 600);
        setCanvasStyle(c);
        gPad->SetLogy();
        //  pr->Draw();
    ...
    void compareTofFull(TString path, double m1 = -8.30623, double m2 = 0.0040615, double obj = 51.95, double ttar = 7.423887)
    {
        ...
        const double shift1 = obj + (m1 + ttar) - m1;
        const double shift2 = obj - m2;

        const int nbx = prUncorr->GetNbinsX();
        const double x1 = 0;
        const double x2 = 100;

        auto *hUncorrShifted = new TH1D("hUncorrShifted", "Uncorr shifted", nbx, x1, x2);
        auto *hCorrShifted = new TH1D("hCorrShifted", "Corr shifted", nbx, x1, x2);
        ...
        {
            const double x = prUncorr->GetBinCenter(i);
            const double y = prUncorr->GetBinContent(i);
            if (y > 0)
                hUncorrShifted->Fill(x + shift1, y);
        }
        for (int i = 1; i <= prCorr->GetNbinsX(); ++i)
        {
            const double x = prCorr->GetBinCenter(i);
            const double y = prCorr->GetBinContent(i);
            if (y > 0)
        ...
        setCanvasStyle(c);
        setHistogramStyle(hCorrShifted, "ToF [ns]", "# counts", kCyan - 6);
        setHistogramStyle(hUncorrShifted, "ToF [ns]", "# counts", kOrange - 3);

        hCorrShifted->Draw("HIST");
        hUncorrShifted->Draw("HIST SAME");

        gPad->SetLogy();

        auto *leg = new TLegend(0.8, 0.85, 0.95, 0.95);
        leg->AddEntry(hCorrShifted, "New params", "l");
        leg->AddEntry(hUncorrShifted, "Old params", "l");
        leg->Draw();

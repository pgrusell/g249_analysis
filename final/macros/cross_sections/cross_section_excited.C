...
void cross_section_excited()
{

    // Data files
    std::string unreactedFilePath = "/nucl_lustre/pablogrusell/g249/g249_analysis/results/dataFiles/data_25F.root";
    std::string ox23FilePath = "/nucl_lustre/pablogrusell/g249/g249_analysis/results/dataFiles/23O_analyzed.root";
    ...
    TString condErel = Form(" && Erel*1000 > %f && Erel*1000 < %f", Erelmin, Erelmax);
    ...
    auto *hErel = new TH1F("hErel", "Erel", 100, 0, 20);

    int kNumFragments = tr_23o->Draw("Z_frag_est:AoQ_frag", static_cast<TString>("reacted_23o_cutg &&") + condOpa + condErel, "goff");
    tr_23o->Draw("Erel*1000>>hErel", static_cast<TString>("reacted_23o_cutg &&") + condOpa, "");
    std::cout << "Fragments  = " << kNumFragments << std::endl;

    // Efficiencies
    double eff_p2p_tot = 0.566897;
    double eff_neuland = calculateEfficiency(hErel, Erelmin, Erelmax);
    std::cout << "Neuland eff  = " << eff_neuland << std::endl;
    ...
    double xs = -TMath::Log(1. - kNumFragments / nproj / 1. / eff_p2p_tot / eff_neuland) * kMt / kRho / kZTarget / kNa * 1e24 * 1e3;

    double R = kNumFragments / nproj / eff_p2p_tot / eff_neuland;
